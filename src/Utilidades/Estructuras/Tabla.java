...
public final class Tabla {
     ...
     *
     * @param datos Un arreglo de cadenas que representa los datos de la fila.
     */
    public void agregarFila(String... datos) {
        if (datos.length == encabezado.length) filas.add(new Fila(datos));
        else Impresora.imprimirError("Contenido Faltante", "La cantidad de columnas no coincide con el encabezado.");
    }
    /**
    ...
    public void eliminarFila(int indice) {
        if (indice >= 0 && indice < filas.size()) filas.remove(indice);
        else Impresora.imprimirError("Indice fuera de rango", "El indice especificado no es valido.");
    }
    /**
    ...
    public void imprimir() {
        ...
        for (int i = 0; i < filas.size(); i++) tabla[i] = filas.get(i).getDatos();
    ...
    /**
     * Elimina una columna de la tabla en el indice especificado (1-based).
     *
     * @param indice El indice de la columna a eliminar (1-based).
     */
     ...
     */
    public void actualizarDato(int fila, int columna, String nuevoDato) {
        ...
        if (filaIndex >= 0 && filaIndex < filas.size() && columnaIndex >= 0 && columnaIndex < encabezado.length) {
            ...
            datosFila[columnaIndex] = nuevoDato;
            filas.get(filaIndex).setDatos(datosFila);
        } else Impresora.imprimirError("Indice fuera de rango", "Los indices especificados no son validos.");
    }
    /**
     * Borra el dato en la coordenada especificada (1-based).
     * Utiliza el metodo actualizarDato para establecer el valor null en la posicion.
     *
